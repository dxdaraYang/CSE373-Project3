...
public class ArrayHeap<T extends Comparable<T>> implements IPriorityQueue<T> {
    ...
    private int numOfChildren(int parentIndex) {
        ...
        for (int i = 1; i <= NUM_CHILDREN; i++) {
            int childIndex = childrenIndex(parentIndex, i);
            if (childIndex < size && heap[childIndex] != null) {
                num++;
            }
        }
        return num;
    ...
    private void percolateDown(int index) {
        int localIndex = index;
        while (numOfChildren(localIndex) > 0 && lessThanParent(minChildIndex(localIndex))) {
