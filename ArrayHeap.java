...
public class ArrayHeap<T extends Comparable<T>> implements IPriorityQueue<T> {
    ...
    public ArrayHeap() {
        this.heap = makeArrayOfT(NUM_CHILDREN+1);
        this.size = 0;
    }
    ...
    private int minChildIndex(int parentIndex) { // changed minIndex
        int minIndex = childrenIndex(parentIndex, 1);
        for (int i = 2; i < numOfChildren(parentIndex); i++) {
            if (heap[childrenIndex(parentIndex, i)].compareTo(heap[minIndex]) <= 0) {
                minIndex = childrenIndex(parentIndex, i);
            }
        }
        return minIndex;
    }
...
//    }
    ...
    private void percolateDown(int index) {
        int localIndex = index;
        while (numOfChildren(localIndex) != 0 && greaterThanChildren(localIndex)) {
    ...
    private boolean greaterThanChildren(int parentIndex) {
        return(numOfChildren(parentIndex)!=0 && heap[parentIndex].
                compareTo(heap[minChildIndex(parentIndex)]) > 0 );
    }
    
    private int numOfChildren(int parentIndex) {
        int num = 0;
        for(int i = 1; i<4; i++) {
            if(childrenIndex(parentIndex,i) < size) {
                num++;
            }
        }
    ...
    private int indexOf(T item) {
        ...
        for (int i = 0; i < size; i++) {
            if (heap[i].equals(item) || heap[i] == item) {
    ...
    private boolean lessThanParent(T item) {
        return (item != heap[0] && item.compareTo(heap[parentIndex(indexOf(item))]) < 0);
    }
